...
function OpenImage(sImagePage) {
	...
	ImageInfo = window.open(sImagePage, '', 'width=660,height=400,toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no');
...
function OpenPrintPage(sPrintPage) {
	...
	PrintPage = window.open(sPrintPage, '', 'width=816,height=400,toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no');
...
function AddTopictoSearch(sTopic) {
		temp = document.bswbTopicPage.TopicClone.value;
		x = temp.length;
		if (x  > 0) { 
		...
		document.bswbTopicPage.TopicClone.value = temp;
		

}

function updateSubject(textfield) {
	opener.document.bswbSiteSearch.TopicInput.value = textfield.value;
}

function TopicSubmitButtononClick() {
	if (self.document.bswbTopicPage.TopicClone.value.length > 0) {
		updateSubject(self.document.bswbTopicPage.TopicClone);
	}
	self.close();
}
...
function AddAuthortoSearch(sAuthor) {
		temp = document.bswbAuthorPage.AuthorClone.value;
		x = temp.length;
		if (x  > 0) { 
		...
		document.bswbAuthorPage.AuthorClone.value = temp;
...
function updateAuthor(textfield) {
	opener.document.bswbSiteSearch.AuthorInput.value = textfield.value;
}

function AuthorSubmitButtononClick() {
	updateAuthor(self.document.bswbAuthorPage.AuthorClone);
	self.close();
}
...
function Set_Select_Value(SelectName, Value) {
...
function TopicSearch(sTopic) {
	temp = document.bswbSiteSearch.TopicInput.value;
	x = temp.length;
	temp = unescape(sTopic);
	document.bswbSiteSearch.TopicInput.value = temp;
}

function TopicSearchPop(sTopic) {
	temp = document.bswbTopicPage.TopicClone.value;
	x = temp.length;
	temp = unescape(sTopic);
	document.bswbTopicPage.TopicClone.value = temp;
...
function AuthorSearch(sAuthor) {
	temp = document.bswbSiteSearch.AuthorInput.value;
	x = temp.length;
	temp = unescape(sAuthor);
	document.bswbSiteSearch.AuthorInput.value = temp;
}

function AuthorSearchPop(sAuthor) {
	temp = document.bswbAuthorPage.AuthorClone.value;
	x = temp.length;
	temp = unescape(sAuthor);
	document.bswbAuthorPage.AuthorClone.value = temp;
...
function FeatureSearch(sTopic) {
	temp = document.FeatureList.TopicInput.value;
	x = temp.length;
	temp = unescape(sTopic);
	document.FeatureList.TopicInput.value = temp;
}

function cds_submit(cdsform, page){
			document.bottForm.cds_page_id.value=page;
			document.bottForm.submit();
}

function SearchReset() {
	document.bswbSiteSearch.BodyInput.value = "";
	document.bswbSiteSearch.AuthorInput.value = "";
	document.bswbSiteSearch.TitleInput.value = "";
	document.bswbSiteSearch.TopicInput.value = "";
	document.bswbSiteSearch.BodyInput.value = "";
}